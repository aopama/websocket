<template>
  <div class="warp">
  	<Chead :left="true" :cen='true' :title="user_id==uid?'我的粉丝':'Ta的粉丝'"></Chead>
...
<!--             <ButtonF :isFollow='item.follow'></ButtonF> -->
            <button class="btn follow" :class="[item.follow=='0'?'':'on']">
              {{item.follow=='0'?'关注':'互相关注'}}
            </button>
          </div>
        </li>
      </div>
...
import { serchFans } from '@/api/api.js';
export default {
  data() {
    return {
  ...
  methods: {
    onPullingDown(v) {
      ...
      this.current = 1;
      setTimeout(() => {
        serchFans({ user_id: this.uid, current: this.current, index: this.index }).then((res) => {
